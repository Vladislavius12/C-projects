...
void Massinit() {
	cout << "¬ведите кол-во эдементов массива: " << endl;
	cin >> ArraySize;
	cout << endl;
...
int stop(int stoppedMarkers,int markerNum,HANDLE* hThread) {
	int markerToStop;
	cout << endl << "”кажите поток дл€ остановки: " << endl;
	cin >> markerToStop;
...
int main() {
	...
	markerStart = CreateEvent(nullptr, TRUE, FALSE, nullptr);
	for (int i = 0; i < markerNum; i++) {
